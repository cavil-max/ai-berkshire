...
This skill is generated from `skills/deep-company-series.md` so Claude Code, Codex, OpenClaw, opencode, and Hermes users share one canonical workflow.

- Treat `$ARGUMENTS` as the user's request in the current Hermes session.
- When the source mentions Claude-only surfaces such as Task, Agent, WebSearch, Bash, Read, or Write, use the closest Hermes capability available: `terminal` for shell commands and local tools, `web_search` / `web_extract` for research, `read_file` / `write_file` for workspace files, and `skill_manage` for skill operations.
- Use shared project tools from `tools/` in this repository. Prefer running commands from the repository root with paths like `python3 tools/financial_rigor.py ...`; if the current session starts outside the repo, locate the actual checkout path first instead of assuming a fixed home-directory path.
- Before starting research, run the `date` command (via `terminal`) to confirm today's date; treat it as the baseline for "latest" data and state the data cutoff date in the report header. Never assume the current date from training data.
